<template>
  <section id="client-love" class="section section-history">
    <div class="container" v-for="testimonial in testimonials" :key="testimonial.url">
      <div class="row">
        <div
          class="col-sm-4 col-tn-8 col-xs-10 col-tn-offset-2 col-xs-offset-1 col-sm-offset-0 pull-left"
        >
...
export default {
  ...
  data() {
    return {
      ...
      imageLoaded: 0,
      rootUrl:
        "https://website-photographs.s3.amazonaws.com/photos-clientLove/",
      photos: []
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async getText(filename) {
      const url =
        "https://u0f8kizng8.execute-api.us-east-1.amazonaws.com/default/GetPageTextByName?filename=" +
        filename;
      const resp = await axios.get(url);
      const json = await resp.data;
      return json;
